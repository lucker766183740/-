  ...
  <el-dialog :visible.sync="visible" :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmitHandle()">
      ...
      <el-form-item label="附件" prop="fileUrl">
          <ren-image v-model="dataForm.fileUrl"  :imageList="imageList" :limit="1" :id="dataForm.id" ></ren-image>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 获取信息
    getInfo () {
      this.$http.get(`/tea/res/info/${this.dataForm.id}`).then(({ data: res }) => {
        if (res.code !== 0) {
        ...
        this.imageStrToList(res.data.fileUrl)
      }).catch(() => {})
    },
    // 表单提交
    dataFormSubmitHandle: debounce(function () {
      this.$refs['dataForm'].validate((valid) => {
        ...
        this.$http[!this.dataForm.id ? 'post' : 'put']('/tea/res/', this.dataForm).then(({ data: res }) => {
          if (res.code !== 0) {
            return this.$message.error(res.msg)
          }
          this.$message({
