    ...
    >
      ...
      <div style="display: flex">
        <el-form-item style="display: flex">
          ...
          <el-button style="margin-left: 20px" @click="getDataList1()">查询</el-button>
        </el-form-item>
      </div>
    </el-form>
    </el-row>
    <el-row :gutter="100" class="card">
      <el-col :span="5" :offset="1"  v-for="(v,i) in list" :key="i" style="padding:20px 10px;margin-left:10px;">
        <router-link  to="/course-classApplication-detail">
          <el-card>
            <div class="newtrain start">
              <div class="newtrain">
                <img :src='"~@/assets/image/" + (i+1)  + ".png"' class="image">
...
import AddOrUpdate from './classApplication-add-or-update'
export default{
  mixins: [mixinViewModule],
  data(){
    return {
      mixinViewModuleOptions: {
        getDataListURL: this.$http.adornUrl('/teacherResearch/researchFile/list'),  //拉取列表访问的URL
        getDataListIsPage: true,
        deleteURL: this.$http.adornUrl('/teacherResearch/researchFile/delete'),     //删除访问的URL
        deleteIsBatch: true,
        deleteIsBatchKey: 'id' ,  // 删除接口，批量状态下由那个key进行标记操作？
        activatedIsNeed: false
      },
      currentDate: new Date().toLocaleString(),
      list:[
        {title:'实践活动课程',user:'张三',label:'实践',status:"已完成",tag:'success',imageUrl:''},
        {title:'心理学课程',user:'李四',label:'心理学',status:"已完成",tag:'success',imageUrl:''},
        {title:'审讯技术课程',user:'王五',label:'审讯技术课程',status:"已完成",tag:'success',imageUrl:''},
        {title:'现场勘查课程',user:'刘能',label:'现场勘查',status:"审核中",tag:'danger',imageUrl:''},
      ]
