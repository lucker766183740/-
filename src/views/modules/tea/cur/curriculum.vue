  ...
  <div class="container">
    ...
    <el-row :gutter="100" class="card">
      <el-col :span="5" :offset="1"  v-for="(v,i) in dataList" :key="i" style="padding:20px 10px;margin-left:10px;">
        <router-link  to="/curriculum-detail">
          <el-card>
            <div class="newtrain start">
              ...
              <div>
                <p class="whiteNowarp">名称：{{v.name}}</p>
                <span><i class="el-icon-user"></i>{{v.promoter}}</span>
  ...
  data () {
    return {
      mixinViewModuleOptions: {
        createdIsNeed: true,
        getDataListURL: this.$http.adornUrl('/tea/cur/page'),
        getDataListIsPage: true,
        deleteURL: this.$http.adornUrl('/tea/cur/delete'),
        deleteIsBatch: true,
        deleteIsBatchKey: 'id',
