<template>
  ...
  <el-input v-model="showName" placeholder="选择菜单" @focus="treeDialog">
    <el-button slot="append" icon="el-icon-search" @click="treeDialog"></el-button>
  </el-input>
  <el-input :value="value" style="display: none"></el-input>
    <el-dialog :visible.sync="visibleTree" width="360px" :modal="false" title="菜单选择" :close-on-click-modal="false" :close-on-press-escape="false">
...
export default {
  ...
  methods: {
    treeDialog () {
      ...
      this.$nextTick(() => {
        this.getDataList(this.value, this.childrenItem)
      })
    },
    ...
    getDataList (id, child) {
      console.log(child)
      return this.$http.get(child).then(({ data: res }) => {
        if (res.code !== 0) {
